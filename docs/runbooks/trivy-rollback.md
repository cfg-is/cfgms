...
This runbook covers what to do if the currently-pinned Trivy release (`v0.74.0`) is later flagged as compromised, OR if Trivy itself starts producing untrusted results.
...
`v0.74.0` is the project's current post-incident release. CFGMS adopts `v0.74.0` based on the project's release announcement and the SHA-256 verification baked into our install path.

## Detection — when to roll back

Roll back if any of these conditions hold:

1. The advisory page is updated to mark `v0.74.0` as compromised or revoked.
2. A subsequent release publishes its own advisory referencing `v0.74.0` as malicious.
...
This is the same install helper used by `v0.74.0`. It verifies the archive SHA-256 before extraction. The hash above is the upstream-published SHA-256 of `trivy_0.69.3_Linux-64bit.tar.gz` and is hardcoded here so that a runtime fetch of `checksums.txt` (which would defeat the purpose if the supply chain is mid-compromise) is not required.
...
5. `.github/workflows/dependency-pin-check.yml` — `check_version "trivy"` argument and the denylist regex (the v0.69.4–v0.69.6 entries stay; do NOT add v0.74.0 to the denylist on rollback unless the advisory confirms compromise).
...
- Replacing Trivy with another scanner. If `v0.69.3` and `v0.74.0` are both compromised, that is a project-wide incident requiring a different decision (Grype, Snyk OSS, etc.) — not a one-line pin change.
