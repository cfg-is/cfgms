...
(`root/msp-a/client-1`); configuration inherits root to leaf. Storage defaults to the
`flatfile` provider (`sqlite` and `database` are alternatives); secrets live in the OS keychain or SOPS, never in cleartext on
