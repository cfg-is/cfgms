...
> `ctrl-node-2`: with `LoadCredentialEncrypted=`, the plaintext appears at
...
The existing cluster (`ctrl-node-1` on `ctrl-node-1`, `ctrl-node-2`,
`ctrl-node-3`) was provisioned before this decision and holds cleartext `secrets.key` files.
...
(`data-svc-vm:8200`; see `docs/testing/controller-ha-real-cluster-runbook.md` §3).
...
`ctrl-node-2` cannot be unsealed on `ctrl-node-3`).
...
1. Verify that the four HA secrets and `secrets.key` are already stored in OpenBao on
   `data-svc-vm`. (They were introduced there at cluster founding per the runbook §3; this
