...
**Addendum (story #3310, epic #2858):** Sessions are now bound to the channel that issued them. The `session.Session` struct and both durable stores (`session_token_records` in SQLite, `session_token_store` in Postgres) carry a `channel TEXT NOT NULL DEFAULT ''` column. `SetDurableSessionStore` sets `Config.Channel = "cli"` for the CLI manager and `Config.Channel = "web"` for the web session manager. At issuance, `manager.issue` stamps `Session.Channel` from `Config.Channel`. `Validate`, `List`, and `Revoke` all enforce channel isolation: a token issued by one channel is rejected (or not listed / not deleted) by the other. Back-filled legacy records have an empty channel string and are rejected by both managers — this is a deliberate one-time forced re-auth on upgrade. `ErrSessionChannelMismatch` is the internal sentinel for in-memory-path cross-channel validation failures; it maps to the generic `INVALID_SESSION_TOKEN` 401 through the existing middleware fallthrough, so the response is byte-for-byte identical to an invalid-token rejection. Cross-channel `Revoke` returns `ErrSessionNotFound` rather than `ErrSessionChannelMismatch` to avoid disclosing that the session exists on another channel.
