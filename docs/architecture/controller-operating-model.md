...
- **Direct approval** (`handleRegister` with `AlwaysApproveHook`): `buildClaimResponse` calls `StewardStore.RegisterSteward` with `StewardStatusRegistered` before returning the cert bundle.
