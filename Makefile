...
check-architecture:
	...
	if [ -n "$$files" ]; then \
		if echo "$$files" | xargs grep -l "tls\.Config{" 2>/dev/null | grep -v "^pkg/cert/" | grep -v "^pkg/quic/" | grep -v "^pkg/controlplane/providers/" | grep -v "^pkg/dataplane/providers/" | grep -v "^test/e2e/" | grep -v "^features/steward/registration/client_http\.go"; then \
		...
		if echo "$$files" | xargs grep -l "tls\.LoadX509KeyPair" 2>/dev/null | grep -v "^pkg/cert/" | grep -v "^pkg/quic/" | grep -v "^pkg/controlplane/providers/" | grep -v "^pkg/dataplane/providers/"; then \
		...
		if echo "$$files" | xargs grep -l "x509\.NewCertPool" 2>/dev/null | grep -v "^pkg/cert/" | grep -v "^pkg/quic/" | grep -v "^pkg/controlplane/providers/" | grep -v "^pkg/dataplane/providers/" | grep -v "^test/e2e/"; then \
		...
		if echo "$$files" | xargs grep -l "x509\.Certificate{" 2>/dev/null | grep -v "^pkg/cert/" | grep -v "^pkg/quic/" | grep -v "^pkg/controlplane/providers/" | grep -v "^pkg/dataplane/providers/"; then \
	...
	\
	echo ""; \
	echo "📦 Checking direct MQTT imports in features/ (pkg/mqtt is removed)..."; \
	if [ -n "$$files" ]; then \
		if [ -z "$$feature_files" ]; then \
			feature_files=$$(echo "$$files" | grep "^features/" || true); \
		fi; \
		if [ -n "$$feature_files" ]; then \
			mqtt_violations=$$(echo "$$feature_files" | xargs grep -l 'cfgis/cfgms/pkg/mqtt/' 2>/dev/null || true); \
			if [ -n "$$mqtt_violations" ]; then \
				echo "  ❌ Found direct pkg/mqtt import in features/ - pkg/mqtt has been removed"; \
