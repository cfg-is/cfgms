...
// TestGetTraceClient_NoBundleFlag_FailsWithNoCredential proves the API-key
// downgrade path is gone: --no-bundle opts out of bundle discovery, and with
// no active session either, getTraceClient must fail naming the required
// credential instead of silently falling back to an API key (Issue #3688).
func TestGetTraceClient_NoBundleFlag_FailsWithNoCredential(t *testing.T) {
	tmpDir := t.TempDir()
	bundleFilePath := filepath.Join(tmpDir, "admin.bundle.yaml")
	generateTestBundleFile(t, bundleFilePath, "https://bundle-trace-controller.local:9443")
