...
- **Leaving banned-phrase markers in changed files**: Adding `// For now`, `// In a real implementation`, `// would implement`, `// simulate`, `// tracked internally`, `// placeholder implementation` markers in your new code is itself a finding. If the work is genuinely deferred, use the `// Deferred: tracked in #NNN — <one-line summary>` annotation pointing at an open `story` or `epic` issue. Anything else is a stub masquerading as progress.
