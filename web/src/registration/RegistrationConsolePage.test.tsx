...
/*
 * RegistrationConsolePage suite (Story #2934): tab strip composition, default
 * tab, roving tabindex, ArrowLeft/ArrowRight keyboard navigation with
 * wraparound, focus side-effects, and the soon-vs-real panel switch.
 *
 * The page mounts PendingQueueTab on the default tab, which issues a real
 * apiFetch — fetch is stubbed at the global level (never the component) so the
 * page renders against the real PendingQueueTab, not a stand-in.
 */
...
beforeEach(() => {
  ...
  fetchMock.mockReset()
  // Default: the pending fetch never settles, so PendingQueueTab stays in its
  // loading state and tab-strip assertions are not racing an async update.
...
describe('RegistrationConsolePage — tab strip', () => {
  ...
  it('badges the not-yet-available tabs with "soon" and leaves Pending unbadged', () => {
    renderPage()
...
// ── Panel switching (soon flag vs real panel) ─────────────────────────────────
...
describe('RegistrationConsolePage — panel rendering', () => {
  ...
  it('renders the soon placeholder for Tokens', () => {
    renderPage()

    fireEvent.click(tab(/^Tokens/i))

    expect(tab(/^Tokens/i)).toHaveAttribute('aria-selected', 'true')
    expect(screen.getByText(/Tokens is not yet available/i)).toBeInTheDocument()
  ...
  it('restores the Pending panel after visiting a soon tab', () => {
    renderPage()
    ...
    expect(tab(/^Pending/i)).toHaveAttribute('aria-selected', 'true')
    expect(screen.getByTestId('pending-loading')).toBeInTheDocument()
    expect(screen.queryByText(/is not yet available/i)).toBeNull()
  })
