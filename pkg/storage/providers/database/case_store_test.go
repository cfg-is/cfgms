...
// TestDatabaseCaseStore_UpdateCaseCAS_WinnerSucceeds verifies the happy path: a
// CAS update at the correct expected version succeeds and returns the new version.
func TestDatabaseCaseStore_UpdateCaseCAS_WinnerSucceeds(t *testing.T) {
	store := newTestCaseStore(t)
	ctx := context.Background()

	c := makeDBCase("db-case-cas-1", "tenant-a")
	require.NoError(t, store.CreateCase(ctx, c))
...
// TestDatabaseCaseStore_UpdateCaseCAS_StaleVersionLoses is the [REQUIRED TEST]
// regression coverage for Issue #3895 at the storage layer: two writers read the
// same case (version 1), both submit conflicting updates; the first CAS at
// version 1 must win, and the second CAS — still using the now-stale version 1 —
// must lose cleanly (ok=false, no error) without overwriting the winner's write.
func TestDatabaseCaseStore_UpdateCaseCAS_StaleVersionLoses(t *testing.T) {
	store := newTestCaseStore(t)
	ctx := context.Background()

	c := makeDBCase("db-case-cas-2", "tenant-a")
	require.NoError(t, store.CreateCase(ctx, c))
...
// TestDatabaseCaseStore_UpdateCaseCAS_NotFound verifies ok=false, no error, for a
// non-existent case.
func TestDatabaseCaseStore_UpdateCaseCAS_NotFound(t *testing.T) {
	store := newTestCaseStore(t)
	ctx := context.Background()

	c := makeDBCase("db-no-such-case-cas", "tenant-x")
