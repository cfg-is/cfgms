...
func TestNewManagerWithStorage(t *testing.T) {
	tests := []struct {
		...
		{
			name: "with oss composite storage provider",
			setupStorage: func(t *testing.T) (*interfaces.StorageManager, error) {
				// OSS composite storage with temporary directory
				tempDir := t.TempDir()
				return interfaces.CreateOSSStorageManager(tempDir+"/flatfile", tempDir+"/cfgms.db")
			},
			wantErr: false,
		},
	}
