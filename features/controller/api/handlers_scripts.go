...
// convertAuditRecordToAPI converts an internal audit record to API format
func convertAuditRecordToAPI(record *script.AuditRecord) ScriptExecutionInfo {
	return ScriptExecutionInfo{
		...
		ScriptConfig: ScriptConfigInfo{
			Shell:         string(record.ScriptConfig.Shell),
			Timeout:       record.ScriptConfig.Timeout,
			WorkingDir:    record.ScriptConfig.WorkingDir,
			Environment:   record.ScriptConfig.Environment,
			SigningPolicy: string(record.ScriptConfig.SigningPolicy),
			Description:   record.ScriptConfig.Description,
			ContentHash:   record.ScriptConfig.ContentHash,
			ContentLength: record.ScriptConfig.ContentLength,
		},
...
// convertMetricsToAPI converts internal metrics to API format
func convertMetricsToAPI(metrics *script.AggregatedMetrics) ScriptMetricsInfo {
	return ScriptMetricsInfo{
		StewardID:       metrics.StewardID,
		Since:           metrics.Since,
		Until:           metrics.Until,
