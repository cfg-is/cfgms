...
type Server struct {
	...
	router                         *mux.Router
	apiRouter                      *mux.Router // Issue #2373: /api/v1 subrouter stored for lazy route registration
...
func (s *Server) setupRouter() {
	...
	// Issue #2373: store the /api/v1 subrouter so Set* methods can register routes lazily.
	s.apiRouter = api
	...
	// Rollback, reports, and workflow/trigger routes are registered lazily by their
	// respective Set* methods (Issue #2373). setupRouter() runs during New(), before the
	// handlers are wired, so nil-gating here would permanently suppress these routes.
	// SetRollbackManager / SetReportsHandler / SetWorkflowHandler register onto s.apiRouter
	// at call-time, preserving the full /api/v1 middleware chain.
...
// SetWorkflowHandler sets the workflow handler and registers workflow and trigger API routes (Issue #414).
