...
// TestRBACStubRoutesDeregistered verifies that the ten stub RBAC routes (subjects,
// role assignments, permission check) no longer exist and return HTTP 404.
// These routes previously returned 501 via placeholder handlers; after Issue #781
// they are removed entirely from the router.
func TestRBACStubRoutesDeregistered(t *testing.T) {
	server := setupTestServer(t)
	apiKey := NewTestKey(t, server, []string{"rbac:list-subjects", "rbac:create-subject",
		"rbac:read-subject", "rbac:update-subject", "rbac:delete-subject",
		"rbac:read-assignments", "rbac:assign-role", "rbac:revoke-role",
		"rbac:read-permissions", "rbac:check-permission"})

	tests := []struct {
		method string
		path   string
	}{
		{"GET", "/api/v1/rbac/subjects"},
		{"POST", "/api/v1/rbac/subjects"},
		{"GET", "/api/v1/rbac/subjects/some-id"},
		{"PUT", "/api/v1/rbac/subjects/some-id"},
		{"DELETE", "/api/v1/rbac/subjects/some-id"},
		{"GET", "/api/v1/rbac/subjects/some-id/roles"},
		{"POST", "/api/v1/rbac/subjects/some-id/roles"},
		{"DELETE", "/api/v1/rbac/subjects/some-id/roles/some-role"},
		{"GET", "/api/v1/rbac/subjects/some-id/permissions"},
	...
	for _, tt := range tests {
		t.Run(tt.method+" "+tt.path, func(t *testing.T) {
			...
			assert.Equal(t, http.StatusNotFound, rec.Code,
				"deregistered route %s %s must return 404, not 501 or 403", tt.method, tt.path)
		})
	}
}
