...
func TestAlertingManager_MaxAlertsPerDay(t *testing.T) {
	...
	config.AlertInterval = 1 * time.Minute // Interval short relative to the advances below
	...
	// Drive the manager's clock deterministically so we can cross the
	// AlertInterval boundary without sleeping on the wall clock. All advances
	// stay within a 24h window so the per-day counter keeps accumulating.
	clock := time.Now()
	alertManager.now = func() time.Time { return clock }
	ctx := context.Background()
	...
	// Advance past the AlertInterval (still within the same day).
	...
	// Advance past the AlertInterval again (still within the same day).
	clock = clock.Add(1 * time.Hour)
...
func TestAlertingManager_AlertInterval(t *testing.T) {
	...
	// Drive the manager's clock deterministically so we can cross the
	// AlertInterval boundary without sleeping on the wall clock.
	clock := time.Now()
	alertManager.now = func() time.Time { return clock }
	ctx := context.Background()
	...
	clock = clock.Add(config.AlertInterval + time.Millisecond)
...
func TestAlertingManager_ChannelFiltering(t *testing.T) {
	...
	const (
		warningTarget  = "https://example.com/warning"
		criticalTarget = "https://example.com/critical"
		allTarget      = "https://example.com/all"
	)

	// targetsForLevel returns the set of channel targets selected for an alert
	// level, exercising getChannelsForLevel -> shouldSendToChannel end to end.
	targetsForLevel := func(level AlertLevel) map[string]bool {
		got := make(map[string]bool)
		for _, ch := range alertManager.getChannelsForLevel(level) {
			got[ch.Target] = true
		}
		return got
	}

	// Info alerts reach only the Info-threshold channel; the Warning- and
	// Critical-threshold channels filter them out.
	assert.Equal(t,
		map[string]bool{allTarget: true},
		targetsForLevel(AlertLevelInfo),
		"info alert should only reach the info-threshold channel",
	)

	// Warning alerts reach the Warning- and Info-threshold channels but not the
	// Critical-threshold channel.
	assert.Equal(t,
		map[string]bool{warningTarget: true, allTarget: true},
		targetsForLevel(AlertLevelWarning),
		"warning alert should reach warning- and info-threshold channels only",
	)

	// Critical alerts clear every configured threshold.
	assert.Equal(t,
		map[string]bool{warningTarget: true, criticalTarget: true, allTarget: true},
		targetsForLevel(AlertLevelCritical),
		"critical alert should reach all three channels",
	)

	// Breach is the highest level and also clears every threshold.
	assert.Equal(t,
		map[string]bool{warningTarget: true, criticalTarget: true, allTarget: true},
		targetsForLevel(AlertLevelBreach),
		"breach alert should reach all three channels",
	)

	// Directly assert the threshold predicate: an alert level is delivered only
	// when it meets or exceeds the channel minimum.
	assert.True(t, alertManager.shouldSendToChannel(AlertLevelWarning, AlertLevelWarning),
		"level equal to minimum must be delivered")
	assert.True(t, alertManager.shouldSendToChannel(AlertLevelCritical, AlertLevelWarning),
		"level above minimum must be delivered")
	assert.False(t, alertManager.shouldSendToChannel(AlertLevelInfo, AlertLevelWarning),
		"level below minimum must be filtered out")
	assert.False(t, alertManager.shouldSendToChannel(AlertLevelWarning, AlertLevelCritical),
		"level below minimum must be filtered out")
}
