...
import (
	...
	flatfileprovider "github.com/cfgis/cfgms/pkg/storage/providers/flatfile"
...
// real flatfile-based ConfigStore, matching the production wiring in server.go.
func newRealWorkflowTrigger(tb testing.TB) WorkflowTrigger {
	tb.Helper()
	tempDir := tb.TempDir()
	storageManager, err := storageif.CreateOSSStorageManager(
		tempDir+"/flatfile",
		tempDir+"/cfgms.db",
	)
...
func TestNewControllerTriggerManager_ReturnsWiredManager(t *testing.T) {
	storage := &flatfileprovider.FlatFileProvider{}
